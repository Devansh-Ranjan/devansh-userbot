...
async def alive(event): 
       pic = alive_pic if alive_pic else "https://telegra.ph/file/e4d9e82a3e9f758a62ff2.jpg" 
       await bot.send_file( 
         event.chat_id, 
         file=pic, 
         caption=f"Userbot is alive!\nOwner: @my_intrusive_thought_won", buttons=[[Button.inline("Commands", data=b"cmds")]], reply_to=event.id )

@bot.on(events.CallbackQuery(data=b"cmds")) 
async def command_buttons(event): 
          await event.edit( 
            "COMMAND MENU\nTap buttons to see categories:", 
            buttons=[ 
              [Button.inline("Fun", b"fun"), Button.inline("Utility", b"util")], 
              [Button.inline("AFK", b"afk"), Button.inline("Alive", b"alive")], 
              [Button.inline("Back", b"back")] ] )

@bot.on(events.CallbackQuery(data=b"fun")) 
async def fun_cmds(event): 
           await event.edit( 
             "Fun Commands\n.afk, .lol, .cry, .tableflip, .hack, .type, .joke", 
             buttons=[Button.inline("Back", b"cmds")] )

@bot.on(events.CallbackQuery(data=b"util")) 
async def util_cmds(event): 
  await event.edit( 
    "Utility Commands\n.id, .ping, .time, .weather <city>, .ss <url>", 
    buttons=[Button.inline("Back", b"cmds")] )

@bot.on(events.NewMessage(pattern=r"^.setalive$")) 
async def set_alive(event): 
   if event.reply_to_msg_id: 
     ...
     if reply.photo: 
       ...
       global alive_pic 
       alive_pic = path 
  await event.reply("Alive picture updated!") 
else: 
   await event.reply("Reply to a photo to set it as alive.") 
else: 
  await event.reply("Reply to a photo to set it as alive.")
...
async def set_afk(event): 
  ...
  if reason: 
    afk_reason = reason 
    await event.reply(f"AFK activated. Reason: {afk_reason}")
